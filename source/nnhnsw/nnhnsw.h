...
// 返回当前簇中所有的簇
std::vector<std::shared_ptr<Cluster>> calculate_clusters(std::shared_ptr<Cluster> &cluster)
{
    uint64_t hit = 0;
    ...
    for (auto &vector : cluster->vectors)
    {
        ...
        {
            ++hit;
            auto temporary_cluster = std::make_shared<Cluster>(cluster->layer);
            vector.second->cluster = temporary_cluster;
            temporary_cluster->vectors.insert(std::make_pair(vector.first, vector.second));
            std::queue<uint64_t> waiting_vectors;
            waiting_vectors.push(vector.first);
            while (!waiting_vectors.empty())
            {
                uint64_t neighbor = waiting_vectors.front();
                waiting_vectors.pop();
                for (auto &out_neighbor : cluster->vectors.find(neighbor)->second->out)
                {
                ...
                for (auto &in_neighbor : cluster->vectors.find(neighbor)->second->in)
                {
                    if (flag.insert(std::make_pair(in_neighbor.first, new_cluster_number)).second)
                    {
                        ++hit;
                        in_neighbor.second.lock()->cluster = temporary_cluster;
                        temporary_cluster->vectors.insert(std::make_pair(in_neighbor.first, in_neighbor.second));
                    }
                }
            }
            new_clusters.push_back(temporary_cluster);
            ++new_cluster_number;
        }
        if (hit == cluster->vectors.size())
        {
            break;
        }
    }
...
// 新的簇中可能没有被选出的向量在上一层中
// 返回需要插入到上一层中的向量
std::vector<std::shared_ptr<Vector_In_Cluster>> divide_a_cluster(std::shared_ptr<Cluster> &cluster)
{
    ...
    if (new_clusters.size() != 1)
    {
        ...
        {
            auto selected_vector = std::make_shared<Vector_In_Cluster>(new_clusters[0]->vectors[0]->global_offset);
                                                                    ...
                                                                    uint64_t top_k = 0)
{
    ...
    // 排队队列
    std::map<float, std::weak_ptr<Vector_In_Cluster>> waiting_vectors;
    // 优先队列
    std::map<float, std::weak_ptr<Vector_In_Cluster>> nearest_neighbors;
...
// todo
template <typename Dimension_Type>
void insert(const Index<Dimension_Type> &index, std::shared_ptr<Vector_In_Cluster> &new_vector,
            const uint64_t target_layer_number)
{
    ...
    every_layer_neighbors.push(
        index->nearest_neighbors(index->vectors[new_vector->global_offset].data,
    ...
    while (!every_layer_neighbors.empty())
    {
        ...
        {
            if (base_distance * distance_rank * index->distance_bound < neighbor_iterator.first)
            {
                break;
            }
            ...
            if (base_cluster == neighbor_vector->cluster.lock())
            {
                ...
                if (neighbor_vector->out.upper_bound(neighbor_iterator.first) != neighbor_vector->out.end())
                {
                    ...
                    if (neighbor_vector->out.begin()->first == neighbor_iterator.first)
                    {
                        ...
                        for (auto iterator = neighbor_vector->out.begin(); iterator != neighbor_vector->out.end();
                             ++iterator)
                        {
                            if (neighbor_iterator.first * offset * index->distance_bound < iterator->first)
                            {
                                ...
                                {
                                    index->insert(selected_vector, target_layer_number + 1);
                                }
                                break;
                            }
                            ++offset;
                ...
                if (every_layer_neighbors.size() != 1 &&
                    base_cluster->selected_vectors.size() <= base_cluster->vectors.size() / index->max_connect)
                {
                                     ...
                                     const std::vector<Dimension_Type> &query_vector, uint64_t top_k)
{
    ...
    else
    {
        ...
        std::multimap<float, std::weak_ptr<Vector_In_Cluster>> every_layer_neighbors =
            nearest_neighbors(index, query_vector, index.layers[0]->clusters[0]->vectors.begin()->second.lock(), top_k);
        // 逐层扫描
        // 因为Vector_InCluster中每个向量记录了自己在下层中对应的向量
        // 所以不需要实际的层和簇
        // 直接通过上一层中返回的结果即可进行计算
        for (auto i = 1; i < index->layers.size(); ++i)
        {
            ...
            for (auto &start_vector : every_layer_neighbors)
            {
                auto temporary_nearest_neighbors =
                    index->nearest_neighbors(query_vector, start_vector.second.lock()->lower_layer.lock(), top_k);
                one_layer_neighbors.insert(temporary_nearest_neighbors.begin(), temporary_nearest_neighbors.end());
                //                for (auto &neighbor : temporary_nearest_neighbors)
                //                {
