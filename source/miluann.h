...
class Vector
{
  public:
    ...
    explicit Vector(const uint64_t id, const std::vector<float> &raw_data) : id(id), data(raw_data)
    {
    }
};
    ...
    const Index &index, const std::vector<float> &target_vector)
{
    ...
    waiting_vectors.push({euclidean2::distance_to_zero(target_vector.data(), index.parameters.dimension),
    ...
    while (!waiting_vectors.empty())
    {
        ...
        {
            nearest_neighbors.push({processing_distance, processing_id});
        }
        else
        {
            ...
            {
                path.insert({processing_distance, processing_id});
                break;
            }
        }
...
bool connected(const Index &index, const Vector &vector, uint64_t id)
{
    auto visited = std::unordered_set<uint64_t>();
    visited.insert(vector.id);
    auto last = std::unordered_set<uint64_t>();
    last.insert(vector.id);
...
void add(Index &index, const uint64_t id, const std::vector<float> &added_vector_data)
{
    ...
    ++index.count;
    // 在索引中创建一个新向量
    index.vectors.insert({id, Vector(id, added_vector_data)});
    auto &new_vector = index.vectors.find(id)->second;
    ...
    while (!nearest_neighbors.empty())
    {
        ...
        auto &neighbor = index.vectors.find(nearest_neighbors.top().second)->second;
        nearest_neighbors.pop();
        // 为新向量添加出边
        new_vector.short_edge_out.insert({distance, neighbor.id});
        // 为邻居向量添加入边
        neighbor.short_edge_in.insert({id, distance});
        // 如果邻居向量的出边小于限制
        if (neighbor.short_edge_out.size() < index.parameters.short_edge_bound)
        {
            // 邻居向量添加出边
            neighbor.short_edge_out.insert({distance, id});
            // 新向量添加入边
            new_vector.short_edge_in.insert({neighbor.id, distance});
        }
        ...
        else if (distance < neighbor.short_edge_out.rbegin()->first)
        {
            ...
            {
                neighbor.keep_connected.insert(farest_id);
                temporary.keep_connected.insert(neighbor.id);
            }
            // 邻居向量添加出边
            neighbor.short_edge_out.insert({distance, id});
            // 新向量添加入边
    ...
    for (auto iterator = path.begin(); iterator != path.end();)
    {
        ...
        auto &vector = index.vectors.find(iterator->second)->second;
        bool add = true;
        for (auto added_edge_iterator = path.begin(); added_edge_iterator != iterator; ++added_edge_iterator)
        {
            if (index.distance_calculation(index.vectors.find(added_edge_iterator->second)->second.data.data(),
                                           vector.data.data(), index.parameters.dimension) < distance)
            {
        ...
        {
            ++iterator;
            vector.long_edge_out.insert({distance, id});
            new_vector.long_edge_in.insert({vector.id, distance});
        }
        else
        {
...
void erase(Index &index, const uint64_t removed_vector_id)
{
    ...
    // 计算分母
    float sum = 0;
    for (auto iterator = removed_vector.long_edge_in.begin(); iterator != removed_vector.long_edge_in.end(); ++iterator)
    {
        auto &distance = iterator->second;
        sum += distance;
    }
                                                                         ...
                                                                         std::vector<uint64_t> &times)
{
    ...
    for (auto iterator = zero_vector.long_edge_out.begin(); iterator != zero_vector.long_edge_out.end(); ++iterator)
    {
        ...
        begin = std::chrono::high_resolution_clock::now();
        auto &t_v = index.vectors.find(neighbor_id)->second;
        end = std::chrono::high_resolution_clock::now();
