...
float calculate_cosine(const float *vector1, uint64_t dimension, uint64_t real_dimension)
{
    ...
    while (vector1_pointer < end)
    {
        ...
        vector1_pointer += 16;
        product = _mm512_add_ps(product, part_vector1, part_vector2);
    ...
    while (vector1_pointer < end)
    {
        ...
        vector1_pointer += 8;
        product = _mm256_add_ps(product, part_vector1);
    ...
    while (vector1_pointer < end)
    {
        ...
        vector1_pointer += 4;
        product = _mm128_add_ps(product, part_vector1);
    ...
    return product / std::sqrt(square_sum1 * real_dimension);
#endif
}

float calculate_distance_to_1(const float *vector1, uint64_t dimension)
{
#if defined(__AVX512F__)
    auto *vector1_pointer = vector1;
    float __attribute__((aligned(64))) temporary_result[16];
    const float *end = vector1_pointer + dimension;
    __m512 difference, part_vevtor1;
    __m512 part_vevtor2 = _mm512_set1_ps(1);
    ...
    const float *end = vector1_pointer + dimension;
    __m256 difference, part_vector1;
    __m256 part_vector2 = _mm256_set1_ps(1);
    ...
    const float *end = vector1_pointer + dimension;
    __m128 difference, part_vector1;
    __m128 part_vector2 = _mm128_set1_ps(1);
    ...
    {
        square_distance += std::pow(vector1[i] 1, 2);
    }
...
class Vector
{
  public:
    ...
    // 向量和1的cosine值
    float cosine;
    // 根据向量的余弦值和索引的参数切分后，向量落在哪个区间
    uint64_t cosine_number;
    // 向量和1的距离
    float distance_to_zero;
    // 指向的邻居向量
    std::multimap<float, uint64_t> out;
    // 向量的邻居
    std::unordered_map<uint64_t, uint64_t> edges;

    explicit Vector(const uint64_t id, const uint64_t global_offset, const uint64_t offset, float cosine,
                    float distance_to_zero, uint64_t cosine_number)
        : id(id), global_offset(global_offset), offset(offset), cosine(cosine), cosine_number(cosine_number),
...
class Sub_Index
{
  public:
    ...
    //
    std::vector<std::map<float, uint64_t>> cosine_and_distance;
    //
    std::set<uint64_t> flag;

    explicit Sub_Index(const uint64_t max_count_bound) : count(0)
    {
...
class Index_Parameters
{
  public:
    ...
    explicit Index_Parameters(const uint64_t dimension, const uint64_t sub_index_bound,
                              const Distance_Type distance_type, const uint64_t relaxed_monotonicity,
                              const uint64_t minimum_connect_number, const uint64_t cosine_number)
        : dimension(dimension), sub_index_bound(sub_index_bound), distance_type(distance_type),
          relaxed_monotonicity(relaxed_monotonicity), minimum_connect_number(minimum_connect_number),
          cosine_number(cosine_number)
    {
    }
};
...
class Index
{
  public:
    ...
    explicit Index(const Distance_Type distance_type, const uint64_t dimension, const uint64_t minimum_connect_number,
                   const uint64_t relaxed_monotonicity, const uint64_t sub_index_bound, const uint64_t cosine_number)
        : parameters(dimension, sub_index_bound, distance_type, relaxed_monotonicity, minimum_connect_number,
                     cosine_number),
          count(0), calculation_dimension(dimension)
    {
        //
...
bool connected(const Index &index, const Sub_Index &sub_index, const uint64_t start,
               std::unordered_map<uint64_t, std::pair<float, uint64_t>> &deleted_edges)
{
    ...
    for (auto round = 0; round < 4; ++round)
    {
        ...
        {
            for (const auto &neighbor : sub_index.vectors[last_vector_offset].edges)
            {
                                                        ...
                                                        const Vector &inserted_vector, const uint64_t start)
{
    ...
    while (!waiting_vectors.empty())
    {
        ...
        for (auto &vector : sub_index.vectors[processing_vector_offset].edges)
        {
                                                                        ...
                                                                        const uint64_t start)
{
    ...
    waiting_vectors.emplace(
        index.distance_calculation(query_vector, sub_index.vectors[start].data.data(), index.calculation_dimension),
        start);
    flags[start] = true;
    while (!waiting_vectors.empty())
    {
        ...
        for (auto &neighbor : sub_index.vectors[processing_vector_offset].edges)
        {
            // 计算当前向量的出边指向的向量和目标向量的距离
...
uint64_t get_start(const Index &index, const Sub_Index &sub_index, uint64_t cosine_number, float distance)
{
    auto i = sub_index.flag.lower_bound(cosine_number);
    if (i == sub_index.flag.end())
    {
        i = std::prev(i);
    }
    auto j = sub_index.cosine_and_distance[*i].lower_bound(distance);
    if (j == sub_index.cosine_and_distance[*i].end())
    {
        j = std::prev(j);
    }
    return j->second;
...
void add(Index &index, Sub_Index &sub_index, Vector &new_vector)
{
    ...
    new_vector.out = nearest_neighbors_insert(
        index, sub_index, new_vector,
        get_start(index, index.sub_indexes[0], new_vector.cosine_number, new_vector.distance_to_zero));
    for (const auto &edge : new_vector.out)
    {
        new_vector.edges.emplace(edge.second, 1);
    }
    for (const auto &neighbor : new_vector.out)
    {
        auto &neighbor_vector = sub_index.vectors[neighbor.second];
        // 在邻居向量中记录指向自己的新向量
        neighbor_vector.edges.emplace(new_vector.offset, 1);
        // 新向量和邻居向量的距离小于邻居向量已指向的10个向量的距离
        if (neighbor_vector.out.size() < index.parameters.minimum_connect_number)
        {
            neighbor_vector.out.emplace(neighbor.first, new_vector.offset);
            ++neighbor_vector.edges.find(new_vector.offset)->second;
            ++new_vector.edges.find(neighbor.second)->second;
        }
        else
        {
            auto max_distance = neighbor_vector.out.begin();
            ...
            if (neighbor.first < max_distance->first)
            {
                ...
                --neighbor_vector.edges.find(temporary->second)->second;
                if (neighbor_vector.edges.find(temporary->second)->second == 0)
                {
                    neighbor_vector.edges.erase(temporary->second);
                }
                --sub_index.vectors[temporary->second].edges.find(neighbor.second)->second;
                if (sub_index.vectors[temporary->second].edges.find(neighbor.second)->second == 0)
                {
                    sub_index.vectors[temporary->second].edges.erase(neighbor.second);
                }
                neighbor_vector.out.erase(temporary);
            }
        }
    }
    if (!connected(index, sub_index, new_vector.offset, deleted_edges))
    {
        for (const auto &edge : deleted_edges)
        {
            sub_index.vectors[edge.second.second].out.emplace(edge.second.first, edge.first);
            if (sub_index.vectors[edge.second.second].edges.contains(edge.first))
            {
                ++sub_index.vectors[edge.second.second].edges.find(edge.first)->second;
            }
            else
            {
                sub_index.vectors[edge.second.second].edges.emplace(edge.first, 1);
            }
            if (sub_index.vectors[edge.first].edges.contains(edge.second.second))
            {
                ++sub_index.vectors[edge.first].edges.find(edge.second.second)->second;
            }
            else
            {
                sub_index.vectors[edge.first].edges.emplace(edge.second.second, 1);
            }
        }
    }
}
                                                      ...
                                                      uint64_t relaxed_monotonicity = 0)
{
    ...
    //    }
    auto cosine = calculate_cosine(query_vector, index.calculation_dimension, index.parameters.dimension);
    auto cosine_number = (uint64_t)((cosine + 1) / (2 / (float)index.parameters.cosine_number));
    return nearest_neighbors_query(index, index.sub_indexes[0], query_vector, top_k, relaxed_monotonicity,
                                   get_start(index, index.sub_indexes[0], cosine_number,
                                             calculate_distance_to_1(query_vector, index.calculation_dimension)));
}
...
void insert(Index &index, const uint64_t id, const float *inserted_vector)
{
    ...
    ++index.count;
    auto cosine = calculate_cosine(inserted_vector, index.calculation_dimension, index.parameters.dimension);
    auto cosine_number = (uint64_t)((cosine + 1) / (2 / (float)index.parameters.cosine_number));
    auto distance_to_1 = calculate_distance_to_1(inserted_vector, index.calculation_dimension);
    if (inserted_vector_global_offset % index.parameters.sub_index_bound == 0)
    {
        ...
        index.sub_indexes.back().vectors.emplace_back(id, inserted_vector_global_offset, index.sub_indexes.back().count,
                                                      cosine, distance_to_1, cosine_number);
        ++index.sub_indexes.back().count;
        index.sub_indexes.back().vectors.back().data.resize(index.calculation_dimension, 0);
        memcpy(index.sub_indexes.back().vectors.back().data.data(), inserted_vector,
               sizeof(float) * index.parameters.dimension);
        index.sub_indexes.back().cosine_and_distance.resize(index.parameters.cosine_number);
        index.sub_indexes.back().cosine_and_distance[cosine_number].emplace(distance_to_1,
                                                                            index.sub_indexes.back().count - 1);
        index.sub_indexes.back().flag.emplace(cosine_number);
    }
    else
    {
        index.sub_indexes.back().vectors.emplace_back(id, inserted_vector_global_offset, index.sub_indexes.back().count,
