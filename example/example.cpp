...
std::vector<std::pair<int32_t, std::vector<int32_t>>> load_result_from_ivecs(const char *filename)
{
    ...
    while (!file.eof())
    {
        ...
        result.push_back(std::pair<int32_t, std::vector<int32_t>>(number, temporary));
    }
    file.close();
...
uint64_t verify(const std::pair<int32_t, std::vector<int32_t>> &result,
                const std::map<float, int32_t> &query_result)
{
    uint64_t hit = 0;
